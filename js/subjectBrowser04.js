...
// thanks http://www.jstree.com/
   ...
   // create  final member: doesn't get children but can get leaves
	...
	for (i=0; i < classarray.length; i++){
		...
		displaychildname = lastBranchOnly(kidClass);
		...
		linkspan = document.createElement("span");
   		...
   		var kidslcclass = lcclass + " -- " + kidClass; // add the child to the subj
   		fetchrel = "fetchAndCheckChildClasses('" + ("treelist" + i) + "','CHILDREN','" + kidClass + "')";
		linkspan.setAttribute("onclick",fetchrel);
		linkspan.setAttribute("title",kidClass);
...
function createBookButton(subj,count){
    ...
    img.setAttribute("src","images/leaves.png");
